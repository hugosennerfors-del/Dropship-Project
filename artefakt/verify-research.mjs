...
const val = await p.$$eval("#rCountry option", (o) => o.map((x) => x.value));
T("marknadsväljaren har de köpstarka först", val.slice(0, 4).join(",") === "US,GB,AU,EU", JSON.stringify(val));
T("förvalt land är inte Sverige", (await p.$eval("#rCountry", (e) => e.value)) !== "SE", await p.$eval("#rCountry", (e) => e.value));
